 ...
 */
class LocalShellSession extends SessionInterface {
  cfg: any = null
  connId: any = null
  ...
  async duplicate() {
  const session = new LocalShellSession(this.cfg)
  session.init()
  return session
  }
  ...
  if (this.current_cols_rows) {
    const { cols, rows } = this.current_cols_rows
    this.resize_window(cols, rows)
  }
  }
}

async function createLocalShellSession(params: any) {
  const session = new LocalShellSession(params)
  session.on('error', (err) => {
  console.error('LocalShellSession error:', err)
  })
  session.init()
  return session
}

registerSessionFactory(SESSION_TYPES.LOCALSHELL, createLocalShellSession)
