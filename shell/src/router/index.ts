...
router.push = (to) => originalPush(to).catch((err) => { console.error('Router push error:', err, 'to:', JSON.stringify(to)); return err })
router.replace = (to) => originalReplace(to).catch((err) => { console.error('Router replace error:', err, 'to:', JSON.stringify(to)); return err })
