...
const DANGEROUS_PATTERNS = [
  ...
  { pattern: /rm\s+-[\dA-QS-Z_]*R\w*\s+/i, reason: 'Recursive force delete' },
  { pattern: /rm\s+-[\dA-EG-Z_]*F\w*\s+/i, reason: 'Recursive force delete' },
  ...
  { pattern: /git\s+push\s+(?:\S.*)?--force/i, reason: 'Force push overwrites remote history' },
  ...
  { pattern: /userdel\s+(?:\S.*)?-r/i, reason: 'Delete user with home directory' },
...
const WARNING_PATTERNS = [
  ...
  { pattern: /mv(?:\s+\S.*(?:[\n\r\u2028\u2029]\s*|[\t\v\f \xA0\u1680\u2000-\u200A\u202F\u205F\u3000\uFEFF])|\s{2,})\//i, reason: 'Move file to system path' },
  { pattern: /cp(?:\s+\S.*(?:[\n\r\u2028\u2029]\s*|[\t\v\f \xA0\u1680\u2000-\u200A\u202F\u205F\u3000\uFEFF])|\s{2,})\/etc\//i, reason: 'Copy to system config directory' },
 ...
 */
export function gradeCommand(command: string) {
 ...
 * @returns {boolean}
 */
export function requiresConfirmation(command: string) {
 ...
 * @returns {string}
 */
export function riskLabel(level: string, locale: string = 'zh-CN') {
  const labels = {
  'zh-CN': {
    safe: '🟢 只读',
    warning: '🟡 有副作用',
  ...
  return ((labels as any)[locale] || labels['zh-CN'])[level] || level
}
