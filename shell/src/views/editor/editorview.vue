...
const emit = defineEmits(['remove-session'])
...
const zoom = () => (fontSize.value = 1.1 ** scale.value * 16)
...
function searchDown() {
  if (!searchCursor.value || !editorInstance.value) {
    return
  }
...
<template>
  <div class="pt-vnc-view">
    <div class="n-editor-toolbar">
      <div class="n-editor-toolbar__nav">
        <div class="nav-wrapper">
          <FolderOpened style="width: 18px; height: 18px; flex-shrink: 0" />
          <span class="editor_url">{{ currentPath }}</span>
        </div>
      </div>
      <div class="n-editor-toolbar__actions">
        <el-select v-model="editorTheme" placeholder="请选择主题">
          <el-option v-for="(theme, index) in themes" :key="index" :label="theme.name" :value="index" />
        </el-select>
        <el-input
    ...
    <div ref="editorRef" class="screen" :style="{ 'font-size': `${fontSize}px` }" />
  </div>
</template>
