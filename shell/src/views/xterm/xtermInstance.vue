...
export default {
  ...
  methods: {
    ...
    async openSFTP() {
      ...
      const sftCfg = { ...sessionConfig }
      sftCfg.config = { ...sftCfg.config, sftpDirt, connId }
    ...
    init() {
      ...
      const sessionCfg = this.$sessionManager.getSessionConfigByInstanceId(this.sessionInstanceId)
    ...
    _updateConfig(newConfig) {
      ...
      const sessionCfg = this.$sessionManager.getSessionConfigByInstanceId(this.sessionInstanceId)
      if (sessionCfg) {
        const oldConfig = merge(sessionCfg.config, newConfig)
    ...
    async handleFileDrop(files) {
      for (let i = 0; i < files.length; i++) {
        const file = files[i]
        try {
          const _type = file.isDir ? 'dir' : 'file'
    ...
    handleCopy() {
      const s = this.$refs.xterm?.getSelection()
      if (!s) {
        return
      }
    ...
    handlePaste() {
      const s = powertools.clipboardReadText()
      if (!s) {
        return
      }
    ...
    handleProperty() {
      const sessionId = this.sessionInstance?.getId()
      if (!sessionId) {
        return
      }
      const sessionCfg = this.$sessionManager.getSessionConfigByInstanceId(sessionId)
      if (!sessionCfg || !sessionCfg.config) {
        return
      }
      const protocol = sessionCfg.config.protocal
    ...
    handleSearch() {
      ...
      const uri = `https://cn.bing.com/search?q=${s}`
      this.openLink(uri)
    },
    ...
    getTheme(themeName) {
      ...
      if (['Tomorrow', 'Spring'].includes(themeName)) {
    ...
    openLogFileInFolder() {
      const file = powertools.getLogDirty()
  ...
  <div class="xterm-instance" :style="{ 'background-color': backgroundColor }" @mousedown.middle="handleSelectPaste">
    ...
    <div class="xterm-main-area">
      <PtXterm
        ...
        class="xterm-pt"
        :send-to-all-term="keyboardToAll"
        :options="options"
        @xterm-focus="xtermFocus"
        @send-to-all="openSendAll"
        @line-data="handleLog"
      ...
      <AiAssistantPanel ref="aiAssistantPanel" @insert-command="handleInsertCommand" />
      <SysMonitor ref="sysMonitor" :session-instance="sessionInstance" />
      <component :is="sessionModalComponent" ref="sessionModalRef" />
    </div>
  </div>
</template>
