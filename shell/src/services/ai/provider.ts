...
const providers: any = { ollama, openai }
 ...
 */
export async function chat(messages: any, onChunk?: (_chunk: string) => void) {
 ...
 */
export async function explainError(context: any, onChunk?: (_chunk: string) => void) {
 ...
 * @returns {Promise<string>}
 */
export async function generateCommand(naturalLanguage: string, context: any, onChunk?: (_chunk: string) => void) {
    const systemPrompt = buildSystemPrompt(context)
    const messages = [
        { role: "system", content: systemPrompt },
