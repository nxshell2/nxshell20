...
export class xzmodem {
  ...
  this.sending = true
  const fileStats = await fsClient.stat(filePath)
  const fileName = await fsClient.basename(filePath)
  const fileObj = {
    name: fileName,
    size: fileStats.size
  }
  ...
  zsession.send_offer(fileObj).then(async(xfer) => {
    ...
    const r_handle = await fsClient.open(filePath, 'r')
    let position = 0
  ...
  zsession.on('offer', async(offer) => {
    ...
    const real_path = await fsClient.pathresolve(savePath, details.name)
    const w_handle = await fsClient.open(real_path, 'w')
    let position = 0
  ...
  const s = `${name} ${percent}% ${bytesHuman(offset, 2)}/${bytesHuman(size, 2)}\r`
  this.zmodemWrite(s)
  if (offset == size) {
    // clear progress
    const c = '\r'.padStart(s.length + 3, ' ')
    this.zmodemWrite(c)
  }
  }
