...
async function handleKeySelect(keyName) {
  if (!keyName) {
    return
  }
  ...
  if (keyName === customFileLabel.value) {
    return
  }
  try {
...
function showModal(sessionId) {
  ...
  if (sessionId) {
    ...
    if (sessionConfig) {
      ...
      if (config && Object.hasOwn(config, 'forwardInRemoteHost') && !['127.0.0.1', 'localhost'].includes(config.forwardInRemoteHost)) {
...
<template>
  ...
  >
    <template #base>
      ...
      <el-row v-if="formData.proxy === 'socksv5'" :gutter="5">
        ...
        <el-col :xs="8" :sm="8" :md="6" :lg="6" :xl="4">
          <el-form-item :label="t('home.profile.auth.socksv5.port.title')">
            <el-input-number v-model="formData.proxyPort" size="small" controls-position="right" :placeholder="t('home.profile.auth.socksv5.port.placeholder')" />
          </el-form-item>
        </el-col>
      </el-row>
        ...
        <el-col :span="12">
          ...
          <el-form-item v-if="formData.authType === 'cert'" :label="t('home.profile.auth.publickey.title')">
            <div style="display: flex; gap: 8px; width: 100%;">
              ...
              <input ref="keyFileInput" type="file" style="display: none" @change="handleKeyFileChange">
              <el-button size="small" @click="sshKeyManagerRef?.show()">
                {{ t('home.settings.ssh-keys.manage') }}
              </el-button>
            </div>
          </el-form-item>
        </el-col>
    ...
    <template #portForward>
      <el-form-item :label="t('home.profile.auth.forwardin.title')">
        <div class="n-port-forward template">
          ...
          <div class="options">
            <el-button type="primary" @click="addForward">
              {{ t('components.session.port.add') }}
            </el-button>
          </div>
        </div>
        <el-scrollbar v-if="formData.forwardIn.length" style="max-height: 260px">
          <div v-for="(item, index) in formData.forwardIn" :key="index" class="n-port-forward">
            <div class="forward-local">
              <div class="host">
                {{ item.localHost }}
              </div>
            ...
            <div class="options">
              <el-button link icon="Delete" @click="removeForward(index)" />
            </div>
          </div>
        </el-scrollbar>
      </el-form-item>
