        ...
        menus: {
          dir: [
            ...
            },
            {
              ...
              type: 'normal',
              handler: this.handleContextMenu_Prop.bind(this)
            }
          ],
          file: [
            {
          ...
          blank: [
            {
              ...
              type: 'normal',
              handler: this.handleContextMenu_UploadFile.bind(this)
            },
            {
              label: 'home.fileview.context-menu.upload-folder',
              type: 'normal',
              handler: this.handleContextMenu_UploadFolder.bind(this)
            },
            {
              type: 'separator'
            },
            {
              ...
              type: 'normal',
              handler: this.handleContextMenu_CreateDir.bind(this)
            },
            /* {
  ...
  methods: {
      ...
      this.fileItems = List.map((dirent) => {
        ...
        return {
          ...
          lastModify: this.$t('home.fileview.mainview.columns.fmt-time', [
            this.formatDateTime(dirent.getMTime())
          ]),
    ...
    async readLink(filePath) {
      ...
      try {
        ...
        const stat = await fsInstance.lstat(realPath)
        const dirent = new Dirent(realPath, stat)
        return dirent
      } catch(err) {
      }
      return false
    ...
    doSearch() {
      const searchReg = new RegExp(this.searchKeyWords, 'i')
    ...
    async goTo(dirPath) {
      const success = await this.readdir(dirPath)
      if (!success) {
        return
      }
    ...
    async replace(destPath) {
      ...
      this.history.stack[this.history.current] = destPath
    },
    ...
    handleFileItemContextMenu(evt, idx) {
      if (idx === undefined) {
        this.contextMenu.fileItemIdx = -1
        showContextMenu(this.contextMenu.menus.blank, evt)
        return
      }
      ...
      const menu = fileItem.dirent.isDirectory() ? this.contextMenu.menus.dir : this.contextMenu.menus.file
    ...
    async handleContextMenu_Prop() {
      ...
      this.filePropDialog.dirent = {
        ...
        size: dirent.getSize(),
        mtime: this.formatDateTime(dirent.getMTime()),
        user: dirent.getUid(),
        group: dirent.getGid(),
        perms: dirent.getPermsString()
      }
    ...
    _onOpenWithSystemFileChanged(watchId, tempFile, remotePath) {
      if (!this._openWithSystemWatch || this._openWithSystemWatch.watchId !== watchId) {
        return
      }
      const now = Date.now()
      if (this._openWithSystemWatch.lastUpload && now - this._openWithSystemWatch.lastUpload < 3000) {
        return
      }
    ...
    async handleOpen(idx) {
      ...
      const dirent = fileItem.dirent
      if (dirent.isDirectory()) {
        const newPath = path.resolve(this.currentPath, dirent.name)
        this.clearSearch()
        this.goTo(newPath)
      } else if (dirent.isSymbolicLink()) {
        const newPath = path.resolve(this.currentPath, dirent.name)
        const stat = await this.readLink(newPath)
        ...
        const cfg = {
          name: `sftpEditor-${dirent.name}`,
          remote_path: path.resolve(this.currentPath, dirent.name),
          ext_name: dirent.name,
          sftp: fs
        }
...
<template xmlns="">
  <div class="pt-file-view">
    <pt-toolbar :center-style="{ overflow: 'hidden', marginLeft: '10px', marginRight: '10px' }">
      <template #left>
        <n-space :size="5">
          <el-tooltip
            class="item"
            ...
            placement="right-end"
          >
            <el-button link icon="Back" :disabled="!canGoBack" @click="goBack" />
          </el-tooltip>
          <el-tooltip
            class="item"
            effect="dark"
            ...
            placement="right-end"
          >
            <el-button link icon="Right" :disabled="!canForward" @click="forward" />
          </el-tooltip>
          <el-tooltip
            class="item"
            effect="dark"
            ...
            placement="right-end"
          >
            <el-button link icon="Top" :disabled="!canGoUp" @click="goUp" />
          </el-tooltip>
          <el-tooltip
            class="item"
            effect="dark"
            ...
            placement="right-end"
          >
            <el-button link icon="Refresh" :disabled="!refresh" @click="refresh" />
          </el-tooltip>
        </n-space>
      </template>
      <template #center>
    ...
    <pt-grid-view
      ...
      @contextmenu="handleFileItemContextMenu"
    ...
    />
    <!-- 查看文件属性弹窗 -->
    <el-dialog v-model="filePropDialog.show" :title="$t('home.fileview.prop-dialog.file-props')" width="40%">
      <div class="file-prop-dialog">
        <el-scrollbar style="height: 100%">
          <el-descriptions
            ...
            :content-style="{ 'height': '32px', 'line-height': '32px' }"
          >
            ...
            <el-descriptions-item :label="$t('home.fileview.prop-dialog.size')" :span="10">
              {{ formatFileSize(filePropDialog.dirent.size) }}
            </el-descriptions-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="handleCloseFilePropDialog">
            {{ $t('components.Cancel') }}
          </el-button>
          <el-button type="primary" @click="handleCloseFilePropDialog">
            {{ $t('components.OK') }}
          </el-button>
        </div>
      </template>
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="handleCreateDirCancel">
            {{ $t('components.Cancel') }}
          </el-button>
          <el-button type="primary" @click="handleCreateDirConfirm">
            {{ $t('components.OK') }}
          </el-button>
        </div>
      </template>
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="handleRenameCancel">
            {{ $t('components.Cancel') }}
          </el-button>
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="handleCloseChmodDialog">
            {{ $t('components.Cancel') }}
          </el-button>
          <el-button type="primary" @click="handleChmodConfirm">
            {{ $t('components.OK') }}
          </el-button>
        </div>
      </template>
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="handleCloseMoveDialog">
            {{ $t('components.Cancel') }}
          </el-button>
