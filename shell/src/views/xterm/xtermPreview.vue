...
export default {
  ...
  watch: {
    'context.xtermTheme': function() {
      this.setTheme(this.getTheme())
    },

    'context.fontWeight': function() {
      this.setOption('fontWeight', this.context?.fontWeight)
    },

    'context.fontSize': function() {
      this.setOption('fontSize', this.context?.fontSize)
    },

    'context.lineHeight': function() {
      if (this.context?.lineHeight !== false && this.context?.lineHeight !== undefined) {
        this.setOption('lineHeight', this.context?.lineHeight)
      }
    },
    'context.letterSpacing': function() {
      if (this.context?.letterSpacing !== false && this.context?.letterSpacing !== undefined) {
        this.setOption('letterSpacing', this.context?.letterSpacing)
      }
    },
    'context.cursorBlink': function() {
      this.setOption('cursorBlink', this.context?.cursorBlink)
    },
    'context.cursorStyle': function() {
      this.setOption('cursorStyle', this.context?.cursorStyle)
    },
    'context.fontFamily': function() {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      this.setTheme(this.getTheme())
      const xtermOptions = ['fontWeight', 'fontSize', 'lineHeight', 'letterSpacing', 'cursorBlink', 'cursorStyle', 'fontFamily']
      for (const key of xtermOptions) {
        if (key in this.context && this.context[key] !== false && this.context[key] !== undefined) {
