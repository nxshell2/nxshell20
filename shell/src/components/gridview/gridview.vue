...
export default {
  ...
  emits: ['open', 'refresh', 'delete', 'go-back', 'change', 'contextmenu', 'file-drop', 'file-sort', 'cut', 'copy', 'paste'],
  ...
  methods: {
    ...
    getViewSize() {
      ...
      const itemWidth = this.itemWidth < 0 ? width - 20 : this.itemWidth
      const itemHeight = this.itemHeight < 0 ? height - 20 : this.itemHeight
      const colSpaceSize = this.itemWidth < 0 ? 0 : this.colSpaceSize
      const rowSpaceSize = this.itemHeight < 0 ? 0 : this.rowSpaceSize
    ...
    handleViewMousemove() {
      ...
      const startCol = Math.floor((this.selectionRect.left - 10 + this.view.scrollX) / itemWidth)
      const endCol = Math.floor((this.selectionRect.right - 10 + this.view.scrollX) / itemWidth)
      const itemHeight = this.itemHeight + this.rowSpaceSize
      const startRow = Math.floor((this.selectionRect.top - mouseYDelta + this.view.scrollY) / itemHeight)
      const endRow = Math.floor((this.selectionRect.bottom - mouseYDelta + this.view.scrollY) / itemHeight)

      for (let i = 0; i < this.items.length; i++) {
        const col = i % this.view.cols
        const row = Math.floor(i / this.view.cols)
    ...
    handleScrollTop() {
      if (this.view.renderStart > 0) {
        ...
        const realLen = this.view.renderStart
    ...
    handleScrollBottom() {
      if (this.view.renderEnd < this._items.length) {
        // scroll down
        let stepCount = Math.floor(this.view.cols * 3)
        const realLen = this._items.length - this.view.renderEnd
        stepCount = stepCount > realLen ? realLen : stepCount
        this.view.renderStart += stepCount
        this.view.renderEnd += stepCount
    ...
    getColumnResizeHandler(idx) {
      const column = this.columnsRenderInfo[idx]
      const handlers = {
        dragStart: () => {
          return true
        },
    ...
    handleContextMenu(evt, idx) {
      this.showItemContextMenu = true
      this.$emit('contextmenu', evt, idx + this.view.renderStart)
    },

    handleViewContextMenu(evt) {
...
<template>
  <div
    ...
    @contextmenu.prevent="handleViewContextMenu($event)"
    @dragover.prevent
    @drop="handleFileDrop"
  >
    <div
    ...
    <div v-if="mode === 'detail'" class="grid-view-header" @contextmenu.stop>
      <div class="columns">
        ...
        >
          {{ $t(column.label) }}
          <span v-mouse-drag="getColumnResizeHandler(idx)" class="resize">|</span>
        </div>
      </div>
    </div>
    <ScrollContainer
    ...
    >
      <div class="item-container" :class="{ detail: mode === 'detail' }">
        ...
        <pt-grid-view-item
          ...
          :width="itemWidth"
          :height="itemHeight"
          :col-space-size="colSpaceSize"
          :row-space-size="rowSpaceSize"
