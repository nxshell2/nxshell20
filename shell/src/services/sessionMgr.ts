...
export class SessionConfig extends EventEmitter {
  ...
  if (!(sessionNode instanceof SessionConfig)) {
    throw new TypeError('SessionNode must be a instance of SessionConfig')
  }
  ...
  if (typeof session === 'number') {
    ...
    const idx = this.subSessions.findIndex((val) => {
...
class SessionManager extends EventEmitter {
  ...
  const registerConfigs = (configs: any[], parentMountId: string) => {
    for (const sessConfig of configs) {
   ...
   * @type {Set}
   */
  const cfgAssociateSet = this.sessionConfigAssociated[cfgId]
  if (cfgAssociateSet) {
   ...
   * @param {boolean} [background]  是否为后台会话，默认为false
   */
  ...
  try {
     ...
     * @type {SessionInterface}
     */
    const instance = await factory.createInstance(createParam)
    const instId = sessInstanceIdGenerator.getNext()
  ...
  const sftp_name = sftpSession.hostName === '' ? sftpSession.hostAddress : sftpSession.hostName
  ...
  const idx = this.sessionInstances.findIndex((val) => {
  ...
  // TODO: add code here
  const instId = fromSessionInstance.getId()
  const sessionCfg = this.getSessionConfigByInstanceId(instId)
  ...
  const instSets = this.sessionConfigAssociated[sessCfg._id]
  if (!instSets || instSets.size === 0) {
    return null
  }

  const matched = []
  for (const inst of this.sessionInstances) {
