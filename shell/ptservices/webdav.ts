...
class WDFileSystem extends PtFileSystem {
  ...
  const list = await this.webDavClient.getDirectoryContents(location)
  return list
  }

  async stat(path: string): Promise<any> {
  const s = await this.webDavClient.stat(path)
  return s
  }
  ...
  const readStrem = await this.webDavClient.createReadStream(filename)
