    ...
    <div v-if="type === 'auth'">
      <el-form label-position="left" label-width="80px" @submit.prevent>
        ...
        <el-form-item v-if="authType === 'password'" :label="$t('components.auth.password.title')">
          <el-input ref="password" v-model="authPassword" type="password" @keydown.enter="doOK" />
        </el-form-item>
        ...
        <el-form-item v-if="authType === 'publickey'" :label="$t('components.auth.passphrase.title')">
          <el-input v-model="authPassphrase" type="password" @keydown.enter="doOK" />
        </el-form-item>
      </el-form>
    </div>
    <el-form v-else-if="type === 'username'" label-position="left" label-width="80px" @submit.prevent>
      <el-form-item :label="$t('components.auth.username.title')">
        <el-input ref="username" v-model="authUserName" type="text" @keydown.enter="doOK" />
      </el-form-item>
    </el-form>
    <div v-else class="prompt">
      <div>{{ promptName }}</div>
      <el-input ref="prompt" v-model="promptValue" type="text" />
    </div>
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="doOK">
          {{ $t('components.OK') }}
        </el-button>
      </div>
    </template>
