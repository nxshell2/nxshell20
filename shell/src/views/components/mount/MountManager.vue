<script>
import { Cloudy, Delete, Edit, Folder, FolderOpened, Monitor, Plus } from '@element-plus/icons-vue'
...
export default {
  ...
  methods: {
    ...
    async handleDelete() {
      if (!this.selectedMount || this.selectedMount.id === 'local') {
        return
      }
      ...
      <div class="mount-list">
        ...
        >
          <div class="mount-icon">
            <el-icon v-if="mount.type === 'local'">
              <Folder />
            </el-icon>
            <el-icon v-else-if="mount.type === 'webdav'">
              <Cloudy />
            </el-icon>
            <el-icon v-else-if="mount.type === 'sftp'">
              <Monitor />
            </el-icon>
            <el-icon v-else>
              <FolderOpened />
          ...
          <div class="mount-info">
            ...
            <div class="mount-type">
              {{ getMountTypeLabel(mount.type) }}
            </div>
          </div>
          <div class="mount-status">
            <el-tag v-if="mount.readonly" size="small" type="info">
              {{ $t('mount.readonly') }}
            </el-tag>
            <el-tag v-if="mount.id === 'local'" size="small" type="success">
              {{ $t('mount.default') }}
            </el-tag>
          </div>
        </div>
          ...
          <el-form-item :label="$t('mount.form.authType')">
            <el-radio-group v-model="editForm.config.authType">
              <el-radio value="password">
                {{ $t('mount.form.authPassword') }}
              </el-radio>
              <el-radio value="publickey">
                {{ $t('mount.form.authKey') }}
              </el-radio>
            </el-radio-group>
          </el-form-item>
