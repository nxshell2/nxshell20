...
class ftpFileObject extends EventEmitter {
  ...
  const copySize = ((position + length) > this.size) ? (this.size - position) : length
...
class FTPFileSystem extends PtFileSystem {
  ...
  let stats = status.find((el: any) => {
    return el.name == basename
  })
  if (stats) {
  ...
  if (flags === 'r') {
    ...
    this.ftp_client.get(filename, (error: any, handle: any) => {
      ...
      const retHanlde = that.handleGenerator.getNext()
      const fileObject = new ftpFileObject(sizeByte)
      that.openedFiles[retHanlde] = handle
      that.openedFileBuffers[retHanlde] = fileObject
      const readStreams = that.openedFiles[retHanlde]
  ...
  return new Promise((resolve, reject) => {
    ...
    const readStreams = this.openedFiles[handle]
    const fileObject: ftpFileObject = this.openedFileBuffers[handle]
  ...
  return await new Promise((resolve, reject) => {
    ...
    const filename = this.openedFiles[handle]
    if (!position) {
