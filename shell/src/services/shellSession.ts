...
class ShellSession extends SessionInterface {
  ...
  try {
    ...
    this.emit('data', `Connect to server failed! ${err && err.message ? err.message : err}\r\n`)
    this.emit('error', `Connect fail: ${err && err.message ? err.message : err}`)
    return
  }
  ...
  async duplicate() {
  const session = new ShellSession(this.cfg)
  session.init()
  return session
  }
...
async function createShellSession(params: any) {
  const session = new ShellSession(params)
  session.on('error', (err) => {
  console.error('ShellSession error:', err)
  })
  session.init()
  return session
}
