...
import type StorageProviderInterface from './storageInterface'
...
class MountManager extends EventEmitter {
  ...
  if (!mountsData || !Array.isArray(mountsData) || mountsData.length === 0) {
    ...
    // 确保本地挂载点存在
    let needSave = false
    if (!this.mounts.some(m => m.id === 'local')) {
  ...
  switch (mount.type) {
    ...
    if (!mount.config) {
      throw new Error('WebDAV config is missing')
    }
    ...
    if (!mount.config) {
      throw new Error('SFTP config is missing')
    }
  ...
  if (mountId === 'local') {
    ...
    mount.icon = updates.icon
    }
    if (updates.readonly !== undefined) {
    mount.readonly = updates.readonly
    }
    if (updates.config !== undefined) {
    mount.config = updates.config
    }
  }
